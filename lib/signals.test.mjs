...
test("candidate validation rejects unsafe names and receipt locators", () => {
  ...
  assert.throws(
    () => signalIdentity(candidate({ receipts: [{ ...candidate().receipts[0], locator: "//user:pass@example.com/message" }] })),
    /credentials/,
  );
  assert.throws(
...
test("higher severity reopens a resolved signal", () => {
  const observed = created(candidate({ severity: "info" }));
  const id = observed[0].signalId;
  const resolved = planSignalTransition(observed, { signalId: id, to: "resolved" }, { now: NOW });
  const plan = ingest([...observed, resolved], candidate({ severity: "critical" }));
  assert.equal(plan.events[0].surfaceReason, "severity-increased");
  assert.equal(plan.events[1].reason, "higher-severity");
  assert.equal(reduceSignalEvents([...observed, resolved, ...plan.events]).get(id).status, "open");
});
...
test("atomic writer reports both write and cleanup failures", async () => {
  const dir = await vault();
  try {
    const events = created();
    await assert.rejects(
      writeSignalEventsAtomic(dir, events, {
        renameImpl: async () => {
          throw new Error("rename failed");
        },
        rmImpl: async () => {
      ...
      (error) => {
        ...
        assert.deepEqual(error.errors.map((item) => item.message), ["rename failed", "cleanup failed"]);
        return true;
      },
    );
  } finally {
