...
test("check prompt keeps captured insight fences inside one context boundary", () => {
  ...
  const open = p.indexOf("<kizuki_vault_context>");
  const insightFence = p.indexOf(fence + "json");
  const close = p.indexOf("</kizuki_vault_context>");
  assert.ok(open !== -1 && open < insightFence && insightFence < close);
  assert.doesNotMatch(p, /```md/);
});
