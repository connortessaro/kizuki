...
async function scanLegacyAlerts(vaultDir) {
  ...
  for (const name of files) {
    ...
    for (let index = 0; index < lines.length; index++) {
      ...
      if (!alert) {
        if (raw.trim() !== "") throw new Error(`${relativePath}:${index + 1}: malformed alert line`);
        continue;
      }
      ...
      try {
        candidates.push({
          candidate: migrationCandidate(alert, {
            relativePath,
...
function groupMigrationCandidates(candidates) {
  ...
  for (const entry of candidates) {
    const { candidate, date } = entry;
    const { signalId } = signalIdentity(candidate);
    const existing = groups.get(signalId);
    if (!existing) {
      groups.set(signalId, {
        candidate: { ...candidate, receipts: [...candidate.receipts] },
        date,
      });
      continue;
    }
    const receipts = [...existing.candidate.receipts, ...candidate.receipts];
    if (date >= existing.date) {
      groups.set(signalId, { candidate: { ...candidate, receipts }, date });
    } else {
      existing.candidate.receipts = receipts;
    }
  }
  return groups;
}
