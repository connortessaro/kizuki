...
function validateArchiveEvent(event, states) {
  ...
  if (state.status === "archived") {
    throw new Error("insight " + event.insightId + " is already archived");
  }
...
export function validateInsightArchiveTransition(transition) {
...
export function planInsightArchive(events, transition, { now = new Date() } = {}) {
  const normalized = validateInsightArchiveTransition(transition);
  const states = reduceInsightEvents(events);
  const state = states.get(normalized.insightId);
  if (!state) throw new Error("unknown insight " + normalized.insightId);
  if (state.status === "archived") throw new Error("insight " + normalized.insightId + " is already archived");
  const event = {
    version: 1,
...
export async function readInsightEvents(
  vaultDir,
...
export async function writeInsightEventsAtomic(
  ...
  reduceInsightEvents(events);
  const existing = await readInsightEvents(vaultDir, { readFile });
  if (
