...
import { parsePayload, extractJsonBlock, trimAlerts, finalizePayload, allClearAlert, parseCheckPayload } from "./payload.mjs";
...
test("parsePayload defaults missing arrays", () => {
  ...
  assert.equal(p.alerts[0].kind, "clear");
});
...
test("finalizePayload adds an all-clear info alert when entities exist but alerts are empty", () => {
  const data = finalizePayload({
  ...
  assert.equal(data.alerts.length, 1);
  assert.equal(data.alerts[0].kind, "clear");
  assert.match(data.alerts[0].evidence, /No cross-team alignment signals/);
  assert.equal(allClearAlert(2).evidence, "No cross-team alignment signals this run; updated 2 entities.");
});
