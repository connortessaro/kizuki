...
async function readHolder(path) {
  ...
  try {
    ...
    if (Number.isInteger(data.pid)) return { raw, data };
  } catch {
    // unparsable lock file — fall through to the stale marker
  }
  return { raw, data: { corrupt: true } };
}
  ...
  for (;;) {
    ...
    if (holder === null) {
      if (Date.now() >= deadline) {
        throw new Error(`vault lock could not be acquired within ${waitMs}ms`);
      }
      continue;
    }
    const { raw, data } = holder;
    if (data.corrupt || !pidAlive(data.pid)) {
    ...
    if (Date.now() >= deadline) {
      throw new Error(`vault locked by ${data.tool} (pid ${data.pid}) since ${data.startedAt}`);
    }
    await sleep(pollMs);
  }
  try {
    ...
    const holder = await readHolder(path);
    if (holder !== null && holder.data.pid === process.pid) await unlinkIfPresent(path);
  }
}
