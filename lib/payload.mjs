...
const KINDS = ["contradiction", "blocker", "mention", "deadline", "clear"];
...
export function allClearAlert(entityCount) {
  const n = entityCount;
  const label = n === 1 ? "entity" : "entities";
  return {
    severity: "info",
    kind: "clear",
    type: "project",
    name: "kizuki",
    evidence: `No cross-team alignment signals this run; updated ${n} ${label}.`,
  };
}
...
export function finalizePayload(data) {
  ...
  if (data.entities.length && !data.alerts.length) {
    data.alerts = [allClearAlert(data.entities.length)];
  }
  return data;
}
