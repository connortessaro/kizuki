...
test("ledger failure happens after consumed transcripts are archived", async () => {
  const v = await makeVault();
  await writeFile(join(v, "transcripts", "m.txt"), "hello", "utf8");
  await assert.rejects(
    applyPayload(v, {
...
test("transcript archive failure leaves the signal ledger uncommitted for retry", async () => {
  const v = await makeVault();
  const archiveTarget = join(v, "transcripts", "processed", "m.txt");
  await writeFile(join(v, "transcripts", "m.txt"), "hello", "utf8");
  ...
  await assert.rejects(applyPayload(v, payload, { now: FIXED }));
  assert.deepEqual(await readSignalEvents(v), []);

  await rm(archiveTarget, { recursive: true });
  const retry = await applyPayload(v, payload, { now: FIXED });
  assert.deepEqual(retry.newAlerts, [signal()]);
  assert.equal((await readSignalEvents(v)).length, 1);
});
