...
test("runDoctor creates missing vault dirs and reports them", async () => {
  ...
  assert.match(r["vault-dirs"].detail, /created people, projects, teams, transcripts, transcripts\/processed, signals/);
...
test("runDoctor reports all-present vault dirs without creating", async () => {
  const dir = await tmp();
  for (const d of ["people", "projects", "teams", "transcripts", "transcripts/processed", "signals"]) {
    await mkdir(join(dir, d), { recursive: true });
  }
  const r = byName(await collect(noSmoke(dir)));
  assert.deepEqual(r["vault-dirs"], { name: "vault-dirs", status: "pass", detail: "6 already present" });
});
...
test("runDoctor checkOnly reports missing dirs as failure and creates nothing", async () => {
  ...
  assert.equal(r["vault-dirs"].detail, "missing: projects, teams, transcripts, transcripts/processed, signals (omit --check-only to create)");
