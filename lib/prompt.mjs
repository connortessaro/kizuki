...
export function buildCheckPrompt({ draft, scope, vaultDir }) {
  const scopeLine =
    scope.kind === "person"
      ? `Only consider the person "${scope.name}".`
      : scope.kind === "project"
        ? `Only consider the project "${scope.name}".`
        : scope.kind === "team"
          ? `Only consider the team "${scope.name}".`
