...
export function buildCheckPrompt({ draft, scope, vaultDir, vaultContext }) {
  ...
  return [
    ...
    vaultContext
      ? `Use only the vault context between the kizuki_vault_context tags below. Do not call tools, read files, or fetch external sources for this check.`
      : null,
    ``,
    ...
    vaultContext ? `<kizuki_vault_context>` : null,
    vaultContext || null,
    vaultContext ? `</kizuki_vault_context>` : null,
    vaultContext ? `` : null,
    `Do the following:`,
