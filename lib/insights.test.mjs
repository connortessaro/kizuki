...
test("identity sorts non-ASCII entity names by stable code units", () => {
  const identity = insightIdentity({
    ...BASE,
    entities: [
      { type: "project", name: "éclair" },
      { type: "project", name: "zebra" },
    ],
  });
  assert.deepEqual(identity.insight.entities.map((entity) => entity.name), [
    "zebra",
    "éclair",
  ]);
});
...
test("reducer rejects malformed event protocol", () => {
  const first = captured();
  const archived = planInsightArchive(
    [first],
    { insightId: first.insightId },
    { now: LATER },
  );
  ...
  assert.throws(
    () => reduceInsightEvents([first, {
      ...planInsightArchive([first], { insightId: first.insightId }, { now: LATER }),
      to: "active",
    }]),
  ...
  assert.throws(
    ...
    /invalid insight actor/,
  );
  assert.throws(
    () => reduceInsightEvents([first, archived, {
      ...archived,
      from: "archived",
      at: "2026-07-09T22:00:00.000Z",
    }]),
    /already archived/,
  );
...
test("atomic writer uses the injected ledger reader before mutation", async () => {
  const vault = await mkdtemp(join(tmpdir(), "kizuki-insights-"));
  await assert.rejects(
    writeInsightEventsAtomic(vault, [captured()], {
      readFile: async () => { throw new Error("injected read failed"); },
    }),
    /injected read failed/,
  );
  assert.equal(await exists(join(vault, "insights")), false);
});
