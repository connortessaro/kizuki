...
export function renderGemini(ritual) {
  if (ritual.body.includes('"""')) {
    throw new Error('gemini ritual body cannot contain a triple quote (""") — not representable in TOML');
  }
...
export const TARGETS = Object.freeze({
  ...
  // Cursor has no native global rules directory — only project-scoped `.cursor/rules/*.mdc`
  // (version-controlled per repo) or UI-only "User Rules" with no file backing. Confirmed via
  // official docs (cursor.com/docs/rules) and a Cursor team reply tracking it as an unshipped
  // feature request. Dist-only, same as generic.
  cursor: {
    render: renderCursor,
    distPath: (name) => join("cursor", name + ".mdc"),
    homePath: null,
  },
  gemini: {
    render: renderGemini,
    distPath: (name) => join("gemini", name + ".toml"),
    homePath: (name, home) => join(home, ".gemini", "commands", name + ".toml"),
  },
  generic: {
    render: renderGeneric,
    distPath: (name) => join("generic", name + ".md"),
    homePath: null,
  },
});
...
function parseExportArgs(argv) {
  ...
  if (!["claude", "codex", "cursor", "gemini", "generic", "all"].includes(agent)) {
    throw new Error("invalid agent " + JSON.stringify(agent));
  }
...
export async function runSkillsCommand(vaultDir, argv, { home = homedir() } = {}) {
  ...
  for (const targetName of targets) {
    ...
    for (const ritual of rituals) {
      ...
      if (!dist && !target.homePath) {
        if (agent !== "all") {
          throw new Error(targetName + " renders to dist only — use --dist or --check");
        }
        continue;
      }
