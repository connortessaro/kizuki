...
- Node >= 20 (this machine: v26). ESM only (`.mjs`, `"type": "module"`). Repo already git-init'd at `~/kizuki`.
...
- Managed-section markers (verbatim): `<!-- KIZUKI:ANALYSIS:START -->` and `<!-- KIZUKI:ANALYSIS:END -->`.
...
**Files:**
- Create: `~/kizuki/package.json`
- Create: `~/kizuki/.gitignore`
- Create: `~/kizuki/people/.gitkeep`, `~/kizuki/projects/.gitkeep`, `~/kizuki/teams/.gitkeep`, `~/kizuki/transcripts/.gitkeep`
- Create: `~/kizuki/README.md`
...
Run:
```bash
cd ~/kizuki && for d in people projects teams transcripts; do mkdir -p "$d" && touch "$d/.gitkeep"; done
```
...
Run:
```bash
cd ~/kizuki && ls -la people projects teams transcripts && node --version
git add -A && git commit -m "chore: kizuki repo skeleton"
```
...
**Files:**
- Create: `~/kizuki/lib/args.mjs`
- Test: `~/kizuki/lib/args.test.mjs`
...
Run: `cd ~/kizuki && node --test lib/args.test.mjs`
...
```bash
cd ~/kizuki && git add lib/args.mjs lib/args.test.mjs && git commit -m "feat: sync argument parsing"
```
...
**Files:**
- Create: `~/kizuki/lib/prompt.mjs`
- Test: `~/kizuki/lib/prompt.test.mjs`
...
Run: `cd ~/kizuki && node --test lib/prompt.test.mjs`
...
```bash
cd ~/kizuki && git add lib/prompt.mjs lib/prompt.test.mjs && git commit -m "feat: codex sync prompt builder"
```
...
**Files:**
- Create: `~/kizuki/lib/payload.mjs`
- Test: `~/kizuki/lib/payload.test.mjs`
...
Run: `cd ~/kizuki && node --test lib/payload.test.mjs`
...
```bash
cd ~/kizuki && git add lib/payload.mjs lib/payload.test.mjs && git commit -m "feat: codex payload parser"
```
...
**Files:**
- Create: `~/kizuki/lib/vault.mjs`
- Test: `~/kizuki/lib/vault.test.mjs`
...
Run: `cd ~/kizuki && node --test lib/vault.test.mjs`
...
```bash
cd ~/kizuki && git add lib/vault.mjs lib/vault.test.mjs && git commit -m "feat: vault markdown helpers"
```
...
**Files:**
- Create: `~/kizuki/lib/apply.mjs`
- Test: `~/kizuki/lib/apply.test.mjs`
...
Run: `cd ~/kizuki && node --test lib/apply.test.mjs`
...
```bash
cd ~/kizuki && git add lib/apply.mjs lib/apply.test.mjs && git commit -m "feat: apply codex payload to vault"
```
...
**Files:**
- Create: `~/kizuki/lib/run.mjs`
- Test: `~/kizuki/lib/run.test.mjs`
- Create: `~/kizuki/sync` (executable)
...
Run: `cd ~/kizuki && node --test lib/run.test.mjs`
...
```bash
cd ~/kizuki && git add lib/run.mjs lib/run.test.mjs sync && git commit -m "feat: sync orchestrator and executable"
```
...
Create `~/kizuki/people/<real-teammate>.md` with a couple of frontmatter fields, and drop a real (or realistic) meeting transcript into `~/kizuki/transcripts/standup-2026-06-30.txt`.

- [ ] **Step 3: Run a scoped dry-run and inspect**

Run: `cd ~/kizuki && ./sync <real-teammate> --source slack --dry-run`
...
Run: `cd ~/kizuki && ./sync <real-teammate> --source slack`
...
Run: `cd ~/kizuki && ./sync <real-teammate> --source slack` again.
...
```bash
cd ~/kizuki && git add people teams projects && git commit -m "chore: seed vault + first real sync"
```
