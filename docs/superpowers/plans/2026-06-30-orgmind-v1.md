# OrgMind v1 Implementation Plan
...
- Node >= 20 (this machine: v26). ESM only (`.mjs`, `"type": "module"`). Repo already git-init'd at `~/orgmind`.
...
- Managed-section markers (verbatim): `<!-- ORGMIND:ANALYSIS:START -->` and `<!-- ORGMIND:ANALYSIS:END -->`.
...
**Files:**
- Create: `~/orgmind/package.json`
- Create: `~/orgmind/.gitignore`
- Create: `~/orgmind/people/.gitkeep`, `~/orgmind/projects/.gitkeep`, `~/orgmind/teams/.gitkeep`, `~/orgmind/transcripts/.gitkeep`
- Create: `~/orgmind/README.md`
...
Run:
```bash
cd ~/orgmind && for d in people projects teams transcripts; do mkdir -p "$d" && touch "$d/.gitkeep"; done
```
...
Run:
```bash
cd ~/orgmind && ls -la people projects teams transcripts && node --version
git add -A && git commit -m "chore: orgmind repo skeleton"
```
...
**Files:**
- Create: `~/orgmind/lib/args.mjs`
- Test: `~/orgmind/lib/args.test.mjs`
...
Run: `cd ~/orgmind && node --test lib/args.test.mjs`
...
```bash
cd ~/orgmind && git add lib/args.mjs lib/args.test.mjs && git commit -m "feat: sync argument parsing"
```
...
**Files:**
- Create: `~/orgmind/lib/prompt.mjs`
- Test: `~/orgmind/lib/prompt.test.mjs`
...
Run: `cd ~/orgmind && node --test lib/prompt.test.mjs`
...
```bash
cd ~/orgmind && git add lib/prompt.mjs lib/prompt.test.mjs && git commit -m "feat: codex sync prompt builder"
```
...
**Files:**
- Create: `~/orgmind/lib/payload.mjs`
- Test: `~/orgmind/lib/payload.test.mjs`
...
Run: `cd ~/orgmind && node --test lib/payload.test.mjs`
...
```bash
cd ~/orgmind && git add lib/payload.mjs lib/payload.test.mjs && git commit -m "feat: codex payload parser"
```
...
**Files:**
- Create: `~/orgmind/lib/vault.mjs`
- Test: `~/orgmind/lib/vault.test.mjs`
...
Run: `cd ~/orgmind && node --test lib/vault.test.mjs`
...
```bash
cd ~/orgmind && git add lib/vault.mjs lib/vault.test.mjs && git commit -m "feat: vault markdown helpers"
```
...
**Files:**
- Create: `~/orgmind/lib/apply.mjs`
- Test: `~/orgmind/lib/apply.test.mjs`
...
Run: `cd ~/orgmind && node --test lib/apply.test.mjs`
...
```bash
cd ~/orgmind && git add lib/apply.mjs lib/apply.test.mjs && git commit -m "feat: apply codex payload to vault"
```
...
**Files:**
- Create: `~/orgmind/lib/run.mjs`
- Test: `~/orgmind/lib/run.test.mjs`
- Create: `~/orgmind/sync` (executable)
...
Run: `cd ~/orgmind && node --test lib/run.test.mjs`
...
```bash
cd ~/orgmind && git add lib/run.mjs lib/run.test.mjs sync && git commit -m "feat: sync orchestrator and executable"
```
...
Create `~/orgmind/people/<real-teammate>.md` with a couple of frontmatter fields, and drop a real (or realistic) meeting transcript into `~/orgmind/transcripts/standup-2026-06-30.txt`.

- [ ] **Step 3: Run a scoped dry-run and inspect**

Run: `cd ~/orgmind && ./sync <real-teammate> --source slack --dry-run`
...
Run: `cd ~/orgmind && ./sync <real-teammate> --source slack`
...
Run: `cd ~/orgmind && ./sync <real-teammate> --source slack` again.
...
```bash
cd ~/orgmind && git add people teams projects && git commit -m "chore: seed vault + first real sync"
```
